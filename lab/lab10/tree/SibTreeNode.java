	...
	public void insertChild(Object item, int c) throws InvalidNodeException {
		...
		if(isValidNode()){
			if(c < 1){
				...
				if(c >= childrenNum + 2){
					insertChild(item, childrenNum + 1);
				} else {
					...
					if(c == 1){
						...
						SibTreeNode currentChild = (SibTreeNode) child(c-1);
						if(c <= childrenNum){
	...
	public void removeLeaf() throws InvalidNodeException {
		...
		if(isValidNode()){
			...
			if(childrenNum == 0){
				...
				SibTreeNode firstSibling = parent.firstChild;
				if(this == firstSibling){
					...
					SibTreeNode currentNode = firstSibling;
