...
class Fraction {
  ...
  /*
	顺带一提，虽然定义Fraction类时，变量numerator和denominator都是
	private类型的，但只要是同类型的object都是可以access这两个private类型的变量的。

	private means that only code within the same class can access the private
	thing. It means private to the class, not private to the object. One dog 
	can see another Dog object's stuff, but a Cat can't see a Dog's private.
