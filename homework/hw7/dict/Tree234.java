...
public class Tree234 extends IntDictionary {
	...
	private void moveUpMidKey(Tree234Node node) {
		/*If node is the root, creates a new root.
		  Else, middle key is placed in parent node.
		  Then splits this node into two nodes.
		*/
		int position = 1;
		if(node == root){
			root = new Tree234Node(node.key2);
			root.child1 = node;
			node.parent = root;
		} else{
			position = node.parent.addKey(node.key2);
		}
		split(node, position);
	}

	private void split(Tree234Node node, int parentPosition) {
		Tree234Node parrentNode = node.parent;
		Tree234Node newNode = new Tree234Node(parrentNode, node.key3);
		switch (parentPosition) {
		case 1:
			parrentNode.child2 = newNode;
			break;
		case 2:
			parrentNode.child3 = newNode;
			break;
		case 3:
			parrentNode.child4 = newNode;
			break;
		}
