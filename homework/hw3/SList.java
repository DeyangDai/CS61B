...
public class SList {
  ...
  public void squish() {
	...
	for(SListNode current = head; current != null && current.next != null; current = current.next){
		while(current.item.equals(current.next.item)){
			size--;
  ...
  public void twin() {
	...
	for(SListNode current = head; current != null;){
