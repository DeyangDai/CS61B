...
public class BinaryTree implements Dictionary {
	...
	private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
		...
		Comparable thisKey = (Comparable) node.entry.key();
		int diff = key.compareTo(thisKey);
		if(diff == 0){
			return node;
		} else if(diff > 0){
	...
	public Entry remove(Object key) {
		...
		if(node.leftChild == null && node.rightChild == null){
			...
			//with 2 children, delete the minimum node in the rightSubTree,
			//and replace it with the node.
		
			BinaryTreeNode rightMinNode = node.rightChild;
			while(rightMinNode.leftChild != null){
				rightMinNode = rightMinNode.leftChild;
			}
			
			//node.entry points to the entry of the minimum node in the rightSubTree (rightMinNode),
			node.entry = rightMinNode.entry;
			
			//then delete rightMinNode.
			if(rightMinNode.rightChild != null){
				BinaryTreeNode child =rightMinNode.rightChild;
				
				BinaryTreeNode parent = rightMinNode.parent;
				child.parent = parent;
				if(parent.leftChild == rightMinNode){
